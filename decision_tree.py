...
def entropy(samples, num_categories):
    ...
    # Avoiding division by zero by handling the case where p_i = 0; as we assume log2(0) is 0 which would give a math error in real calculation
    non_zero_prob = prob[prob != 0]

    # Calculate entropy using the formula: sum(-p_i * log2(p_i))
    ent = np.sum((-1*non_zero_prob) * np.log2(non_zero_prob))
...
def optimal_split(X, y, H_data, split_dim, num_classes, debug=False):
    ...
    # unique_values has all the possible unique splits
    for ii in range(unique_values.shape[0] - 1):

        # Determines the split's index and the candidate on which it is splitting
        split_indx = first_idxes[ii + 1]
        split_val_cand = unique_values[ii]

        # Based on the index, split into two children
        left_child = y[:split_indx]
        right_child = y[split_indx:]

        # Computes the entropy for left and right children
        H_left = entropy(left_child, num_classes)
        H_right = entropy(right_child, num_classes)
        
        # Computes the information gain
        size_left = len(left_child)
        size_right = len(right_child)
        size_total = size_left + size_right
        current_information_gain = H_data - ((size_left / size_total) * H_left + (size_right / size_total) * H_right)
